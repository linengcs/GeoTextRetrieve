...
class BGE_EVAToken(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_layers = self.bge_encoder.config.num_hidden_layers
        self.config = self.bge_encoder.config
        self.model_visual, self.preprocess_train, self.preprocess_val= create_model_and_transforms(
            model_name_eva, 
            eva_pretrained_path, 
    ...
    def t_encoder(self, texts):
        ...
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(attention_mask, input_shape, dtype=embedding_output.dtype)
    ...
    def mm_encoder(self, images, prompts):
        ...
        head_mask = [None] * self.num_layers
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(prom_img_attention_mask, prom_img_input_shape,dtype=prompt_img_embedding.dtype)
    ...
    def img_token_embedding(self, images):
        ...
        # img_token_emb = self.model_visual.forward_features(images, normalize=False,return_all_features=True) #return_all_features=True, [B, T, 768] 
        return img_token_emb.contiguous()
    
    def encode_image_only(self, images, prompts = None):
        '''
        this function is used for Retrival Benchmarking
